    ...
    const renderApp = (req, res) => {
      ...
      renderer.renderToString(context, (err, html) => {
        if (err) {
          ...
          console.error(err)
          // find correct error page
          const errorPage = config.errorPages.find(item => item.code === err.code)
          if (errorPage) {
            res.status(err.code).sendFile(path.resolve(errorPage.template))
          } else {
            // send text as a default
            const code = 500
            res.status(code)
